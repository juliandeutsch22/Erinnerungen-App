...
export class SqliteTaskRepository implements TaskRepository {
  ...
  async create(task: Task): Promise<void> {
    ...
    await db.runAsync(
      `INSERT OR REPLACE INTO tasks
         (id, list_id, title, note, due_date, due_time, rrule, flagged, completed_at, notification_id, created_at, sort)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
  ...
  async update(id: string, patch: Partial<Omit<Task, 'id'>>): Promise<void> {
    ...
    const map: Record<string, string> = {
      ...
      notificationId: 'notification_id', sort: 'sort',
    };
    for (const [key, col] of Object.entries(map)) {
      if (key in patch) {
        const value = (patch as Record<string, unknown>)[key];
        sets.push(`${col} = ?`);
        args.push(key === 'flagged' ? (value ? 1 : 0) : (value as string | number | null));
      }
    }
