...
import { type AssistantAction, describeAenderung, describeExtras, describeSchritte, ortZusatz, resolveTaskHandle } from '@/lib/assistant';
...
export function omniZeilen(a: AssistantAction, tasks: Task[], today: string): OmniZeile[] {
  return [
    ...
    ...a.termine.map((t, i) => ({
      key: `t${i}`,
      titel: t.titel,
      unter: `${formatDueDate(t.datum, today)}${t.start ? ` · ${t.start}` : ' · ganztägig'}${ortZusatz(t)}`,
      art: 'Termin',
      editierbar: true,
    })),
